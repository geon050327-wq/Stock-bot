...
def generate_chart(ticker_symbol):
    try:
        ...
        hist = stock.history(period="6mo") # 6개월치 데이터
        
        if hist.empty: return None

        plt.figure(figsize=(10, 5)) # 그림판 크기 설정
        
        # 1. 주가 선 그리기 (파란색)
        plt.plot(hist.index, hist['Close'], label='Price', color='#1f77b4', linewidth=2)
        
        # 2. 50일 이동평균선 그리기 (주황색, 추세선)
...
for t, my_avg in my_portfolio.items():
    try:
        ...
        if not hist.empty:
            ...
            if chart_buf:
                files[f"{t}.png"] = chart_buf # 전송 목록에 추가
...
payload = {
    "embeds": [{
        "title": "📊 My Portfolio & Charts",
        "description": description,
        "color": 0x2b2d31,
        "fields": embed_fields,
        "footer": {"text": "Charts generated by Python"},
        "timestamp": datetime.now().isoformat()
    }]
}

# ⚠️ 이미지가 있으면 multipart/form-data로 보내야 함
# 이미지가 없으면 그냥 json으로 보냄
if files:
    # 파일을 디스코드 포맷에 맞게 변환
    multipart_files = {}
    ...
    # 임베드에 이미지는 하나만 미리보기로 넣을 수 있음 (첫 번째 종목 차트)
    # 하지만 파일은 다 첨부됨
    requests.post(discord_url, data={"payload_json": str(payload).replace("'", '"')}, files=multipart_files)
else:
    requests.post(discord_url, json=payload)

print("🚀 [전송 완료] 차트가 포함되었습니다!")
